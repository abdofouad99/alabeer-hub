...
function scrapeAdsLibrary(
    ...
    if (!empty($fbData['success'])) {
        ...
        if (!empty($adLib)) {
            $fbAdsActive = !empty($adLib['is_business_page_active']) || (($adLib['ad_count'] ?? 0) > 0);
            $fbAdsCount  = (int)($adLib['ad_count'] ?? 0);
        }
        if (!$fbAdsActive && $adStatus === 'ACTIVE') {
            $fbAdsActive = true;
        }
    }
    ...
    // إذا كان الساحب السريع يقول (لا توجد إعلانات نشطة) والعدد صفر، نكتفي بالنتيجة.
    if (!($fbAdsActive ?? false) && ($fbAdsCount ?? 0) === 0) {
...
function scrapeCompetitorsViaGoogle(string $companyName, string $targetAudience, string $token): array {
    ...
    // استخراج النتائج (أول 5 نتائج Organic)
    $competitors = [];
    foreach ($result as $item) {
        $organic = $item['organicResults'] ?? [];
