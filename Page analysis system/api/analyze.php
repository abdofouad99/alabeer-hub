...
function runAnalysis(int $assessmentId): array {
    ...
    if (($cfg['analysis']['enable_apify'] ?? false)) {
        try {
            Diag::snapshot('competitors.discovery.input', [
                'has_scan_result' => !empty($scanResult),
                'scan_result_keys' => is_array($scanResult) ? array_keys($scanResult) : [],
            ]);
            $compResult = runCompetitorDiscovery($scanResult, $cfg);
            Diag::snapshot('competitors.discovery.result', $compResult);
