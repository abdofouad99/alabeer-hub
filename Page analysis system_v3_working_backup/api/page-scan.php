...
function runPageScan(string $rawUrl, array $cfg): array {
    ...
    if ($type === 'facebook') {
        ...
        if ($detectedFbUrl) {
            ...
            $result['social']   = $fbData;
            $result['facebook'] = $fbData;
            // اكتشاف IG إذا لم يكن موجوداً من الموقع
            if (!$detectedIgUrl) {
    ...
    // ── Step 4: حساب درجة الفحص الجزئية ───────────────────────
...
function scanFacebookPublic(string $url, array $cfg = []): array {
    ...
    // ── 1) Apify Scraper تم إزالته من هنا لتجنب التكرار ─────────────
    // سيعمل فقط في analyze.php عبر الدوال المنفصلة
    ...
    if (preg_match('/([\d.]+\s*[KkMm]?)[\s\u00a0]*(متابع|‎متابع|followers|Followers)/ui', $html, $mFol)) {
        $data['followers'] = function_exists('parseFollowerCount')
            ? (parseFollowerCount($mFol[1]) ?? (int)str_replace(',', '', $mFol[1]))
            : (int)str_replace(',', '', $mFol[1]);
    }
    if (preg_match('/([\d.]+\s*[KkMm]?)[\s\u00a0]*(إعجاب|‎إعجاب|likes|Likes)/ui', $html, $mLikes)) {
...
function scanInstagramPublic(string $url, array $cfg = []): array {
    ...
    if ($json && isset($json['data']['user'])) {
        ...
        $data['posts']      = $u['edge_owner_to_timeline_media']['count'] ?? null;
...
function scanWebsiteHTML(string $url, array $cfg = []): array {
    ...
    // ── 2) محاولة Apify (Puppeteer) كخطة بديلة إذا تم حجب cURL (مثلا بسبب Cloudflare)
    if ((!$html || strlen($html) < 500 || $httpCode >= 400) && ($cfg['analysis']['enable_apify'] ?? false)) {
        require_once __DIR__ . '/apify-scraper.php';
    ...
    return [
        ...
        'has_whatsapp'    => (bool)preg_match('/wa\.me\/|whatsapp\.com|api\.whatsapp/i', $html),
