...
function renderFacebook(fbData) {
  ...
  const rows = [
    ...
    { icon:'❤️', label:'الإعجابات',        val: social.likes       ? formatNum(social.likes)           : '—', highlight: !!social.likes },
    { icon:'⭐', label:'التقييم',          val: social.rating      ? `${social.rating}/5 ⭐`            : '—', highlight: !!social.rating },
    ...
    { icon:'✅', label:'صفحة موثّقة',      val: social.is_verified ? '✅ نعم' : '❌ لا', color: social.is_verified ? '#10b981' : '#94a3b8' },
...
function renderPageInfoBadges(data) {
  ...
  const badges = [
    ...
    scan.url_type === 'facebook'   ? { cls:'badge-purple', text:'📘 فيسبوك' }                 : null,
    scan.url_type === 'instagram'  ? { cls:'badge-purple', text:'📸 انستقرام' }               : null,
    scan.url_type === 'website'    ? { cls:'badge-purple', text:'🌐 موقع' }                   : null,
...
async function downloadPdf() {
  ...
  try {
    ...
    const canvas= await html2canvas(card, { scale: 2.5, backgroundColor: '#050810', useCORS: true });
    const pdf   = new jsPDF('p', 'mm', 'a4');
    const pageW = 210, pageH = 297;
    const imgH  = (canvas.height * pageW) / canvas.width;
    let posY = 0, remaining = imgH;
    while (remaining > 0) {
      pdf.addImage(canvas.toDataURL('image/png'), 'PNG', 0, -posY, pageW, imgH);
      remaining -= pageH; posY += pageH;
      if (remaining > 0) pdf.addPage();
    }
    pdf.save(`بصمة-النمو-${id}.pdf`);
  } catch (e) {
    alert('⚠️ حدث خطأ في تحميل PDF.'); console.error(e);
  } finally {
    btn.disabled = false;
...
function copyLink() {
  navigator.clipboard.writeText(location.href).then(() => {
    ...
    setTimeout(() => btn.textContent = '🔗 مشاركة النتيجة', 2000);
  }).catch(() => prompt('انسخ الرابط يدوياً:', location.href));
}
