...
function buildPreviewMeta(data) {
  ...
  if (social.followers) parts.push(`${formatNum(social.followers)} متابع`);
...
function renderStep() {
  ...
  document.getElementById('stepBadge').innerHTML = `الخطوة <b>${step}</b> / 4`;
  document.getElementById('stepLabel').textContent = STEP_LABELS[step - 1];
  ...
  document.getElementById('btnNext').className    = step === 4 ? 'btn btn-gold' : 'btn';
